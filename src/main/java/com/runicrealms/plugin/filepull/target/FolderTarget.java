...
//    ITEMS(Material.GOLD_NUGGET, "RunicItems/items", "runicitems", "items", false),
...
//    ITEM_PERKS(Material.BEACON, "RunicItems/itemperks", "itemperks", "item-perks", false);
    ...
    public FolderTarget(Material material, String localPath, String githubPath, String identifier, boolean adminsOnly) {
    ...
    public int pull(Consumer<Integer> onProgressUpdate) throws Exception {
        JSONObject commit = (JSONObject) (new JSONParser()).parse(FileUtils.getWithAuth(
                "https://api.github.com/repos/"
                        ...
                        + "/branches/"
                        + RunicFilePull.getInstance().getFileConfig().getTargetBranch()
        ...
        JSONArray tree = (JSONArray) ((JSONObject) (new JSONParser()).parse(FileUtils.getWithAuth(treeURL, RunicFilePull.GH_AUTH_TOKEN))).get("tree");
        ...
        for (Object object : tree.toArray()) {
            ...
            if (((String) treeObject.get("path")).equalsIgnoreCase(githubPath)) {
                ...
                JSONObject folderJSON = (JSONObject) (new JSONParser()).parse(FileUtils.getWithAuth(folderURL, RunicFilePull.GH_AUTH_TOKEN));
            ...
            for (Object object : finalFiles.toArray()) {
                ...
                if (jsonObject.get("type").equals("blob") && ((String) jsonObject.get("path")).endsWith(".yml")) {
                    try {
                        ...
                        JSONObject gitJson = (JSONObject) (new JSONParser()).parse(FileUtils.getWithAuth((String) ((JSONObject) object).get("url"), RunicFilePull.GH_AUTH_TOKEN));
                        FileUtils.writeToFile(FileUtils.decodeBase64((String) gitJson.get("content")), localFile);
